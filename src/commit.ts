...
export const hasConsecutiveAutoGeneratedCommits = async (headSHA: string) =>
  await core.group(
    `Checking the last ${LIMIT_CONSECUTIVE_AUTO_GENERATED_COMMITS} commits to prevent an infinite loop`,
    async () => {
      await git.fetch({ refs: [headSHA], depth: LIMIT_CONSECUTIVE_AUTO_GENERATED_COMMITS })
      ...
      return (
        ...
        lastCommitMessages.every((message) => message.match(/^Auto-generated-by: update-generated-files-action;/))
      )
    },
  )
