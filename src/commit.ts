...
export const failIfInfiniteLoop = async (headSHA: string) => {
  ...
  if (lastCommitMessages.every((message) => message.includes(GENERATED_BY_TRAILER))) {
    throw new Error(`This action has been called ${LIMIT_REPEATED_COMMITS} times. Stop to prevent infinite loop.`)
  }
}
