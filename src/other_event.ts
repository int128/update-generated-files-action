...
export const handleOtherEvent = async (inputs: Inputs, context: Context, octokit: Octokit): Promise<Outputs> => {
  ...
  if (await hasConsecutiveAutoGeneratedCommits(context.sha, context)) {
  ...
  const updated = await core.group(`Trying to update ${context.ref} by fast-forward`, async () => {
    return await updateRefByFastForward(inputs, context)
  })
  if (updated) {
...
const updateRefByFastForward = async (inputs: Inputs, context: Context): Promise<boolean> => {
  const code = await git.push({ localRef: `HEAD`, remoteRef: context.ref, dryRun: inputs.dryRun }, context, {
...
const createOrUpdatePull = async (inputs: Inputs, context: Context, octokit: Octokit): Promise<PullRequest> => {
  const headBranch = ['update-generated-files', ...inputs.headBranchKeys]
    .map((key) => key.replaceAll(/[^\w]/g, '-'))
    .join('--')
  core.info(`Pushing a branch ${headBranch}`)
  await git.push(
    {
      localRef: `HEAD`,
      remoteRef: `refs/heads/${headBranch}`,
      force: true,
  ...
  if (existingPulls.length > 0) {
    ...
    core.info(`Updating ${existingPull.html_url}`)
    if (inputs.dryRun) {
      core.warning(`[dry-run] Updated ${existingPull.html_url}`)
      return existingPull
    }
